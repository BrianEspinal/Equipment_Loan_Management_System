...
const createCatalogApi = <T extends { id: number }>(endpoint: string) => ({
  getAll: async (): Promise<ServiceResult<T[]>> => {
    const response = await apiClient.get<ServiceResult<T[]>>(`/${endpoint}`);
    return response.data;
  },
  getById: async (id: number): Promise<ServiceResult<T>> => {
    const response = await apiClient.get<ServiceResult<T>>(`/${endpoint}/${id}`);
    return response.data;
  },
  create: async (data: Omit<T, 'id'>): Promise<ServiceResult<T>> => {
    const response = await apiClient.post<ServiceResult<T>>(`/${endpoint}`, data);
    return response.data;
  },
  update: async (id: number, data: T): Promise<ServiceResult<T>> => {
    const response = await apiClient.put<ServiceResult<T>>(`/${endpoint}/${id}`, data);
    return response.data;
  },
  delete: async (id: number): Promise<ServiceResult<boolean>> => {
    const response = await apiClient.delete<ServiceResult<boolean>>(`/${endpoint}/${id}`);
    return response.data;
  },
});
